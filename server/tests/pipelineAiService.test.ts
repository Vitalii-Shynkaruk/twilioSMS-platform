...
import {
  ...
  extractDeterministicRequestedAmount,
...
describe('M1.4 Pipeline AI service', () => {
  ...
  it('детерминированно извлекает ask amount из explicit ask phrases', () => {
    expect(extractDeterministicRequestedAmount('Contractor seeking $100k to buy land. Grosses $50k monthly.')).toEqual({
      value_usd: 100000,
      raw: '$100k',
    });
    expect(extractDeterministicRequestedAmount('Needs 100k for business expansion')).toEqual({
      value_usd: 100000,
      raw: '100k',
    });
