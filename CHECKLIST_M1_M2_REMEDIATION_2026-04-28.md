...
- Latest HELOC suggestion repair: commit `8b6b61d` now treats questions like `What is a HELOC?` / `What is a LOC?` as contextual product clarifications, replaces stale email-style suggestions with product-aware explanations, targeted regression `server/tests/aiSuggestionPolicy.test.ts` passed 13/13, and `server npm run build` passed.
...
- Production REP cleanup: temporary account `Vitalii Shynkaruk` (`VS`, `94vshynkaryk@ukr.net`) was safely hard-deleted after verifying `0` dependent leads, conversations, messages, campaigns, assignments, activity logs, tags, deals, deal events, funding events, and renewal tasks.
...
| 2026-04-29 | AI Suggestions        | Repaired stale HELOC / LOC product-definition suggestions   | 8b6b61d    | `aiSuggestionPolicy` 13/13 passed; `server npm run build` passed      | Done    |
| 2026-04-29 | REP Cleanup           | Removed temporary production REP account for Vitalii        | N/A        | Dependency audit all zero; post-check confirms user no longer exists  | Done    |
