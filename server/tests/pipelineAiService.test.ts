...
describe("M1.4 Pipeline AI service", () => {
  ...
  it("выбирает inbound SMS targets только по активным post-intake deals", async () => {
    ...
    expect(findMany).toHaveBeenCalledWith({
      where: {
        OR: [
          { smsConversationId: "conversation-1" },
          { leadId: "lead-1" },
          { id: { in: ["deal-qualified"] } },
        ],
  ...
  it("сериализует extraction queue per deal", async () => {
    ...
    await expect(Promise.all([first, second])).resolves.toEqual([emptySignals, { ...emptySignals, industry: "trucking" }]);
