...
- [ ] После каждой правки обновлять этот checklist: статус, дата, commit SHA, проверка, evidence.
- [ ] Любой production fix сначала переносить в локальный repo и коммитить в `ksanyok/twilio-sms-platform`.
- [ ] Production должен деплоиться только из зафиксированного commit SHA.
...
- [ ] Перед каждым push проверять `git status --short`, `git log -1 --format=fuller`, `git remote -v`.
...
- [ ] Зафиксировать текущий локальный SHA ветки `deploy/mysql-hosting`.
- [ ] Зафиксировать текущий production SHA, `pm2 status`, `NODE_ENV`, `CLIENT_URL`, `WEBHOOK_BASE_URL`.
- [ ] Сохранить список production modified/untracked files до cleanup.
...
- [ ] CTA visible inside AI suggestion card.
- [ ] If lead email exists: CTA is clickable.
- [ ] Click opens PMF Gmail compose in new tab.
- [ ] URL format exactly: `https://mail.google.com/mail/?view=cm&fs=1&to={email}`.
- [ ] `To` field pre-populated with lead email.
- [ ] No subject/body generated by SCL.
- [ ] Rep selects Gmail template in PMF Gmail.
- [ ] SCL never builds/stores/sends emails.
- [ ] If no email: CTA visible but disabled/not clickable.

### 11.2 Current bug investigation

- [ ] Check current `AISuggestions.tsx` after user/formatter changes.
- [ ] Check current `InboxPageV2.tsx` after user/formatter changes.
- [ ] Confirm CTA button is rendered, not plain text.
- [ ] Confirm click handler is wired to Gmail compose.
- [ ] Confirm disabled state depends on email.
- [ ] Confirm screenshot case: `GET EMAIL & SEND FUNDING LINK` should be disabled if no email, or should ask/use captured email flow if required.

### 11.3 Functional tests

- [ ] Conversation with email: click CTA -> new Gmail tab URL contains `to=email` only.
- [ ] Conversation without email: CTA disabled and no tab opens.
- [ ] Card body click still inserts suggestion into SMS compose.
- [ ] `Use`, `Edit`, `Skip` still work.
- [ ] No nested interactive invalid HTML.
- [ ] Keyboard Tab reaches CTA and disabled state is accessible.

Acceptance:

- [ ] Client screenshot scenario explained/fixed.
- [ ] Funding link CTA works exactly as requested.
